...
export default function Game ({searchParams}: {searchParams: {type: string, message?: string}}) {
    ...
    const handleStartGame = async () => {
      if(!selectedProfiles.length) {
        return router.replace(`/game?type=${searchParams.type}&message=Please select at least one participant`)
      }
        ...
        if (!game) {
          ...
          await supabase.from('game_participants').insert(selectedProfiles.map((profile) => ({
            game_id: game.id,
